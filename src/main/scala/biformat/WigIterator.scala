...
import biformat.BlockIterator.{FilteredBlockIterator, GenBlockIterator, MergedIterator}
...
abstract class WigIterator extends BlockIterator[WigUnit]{
  ...
  def filterWithBed(_bit: BedIterator, _wit: WigIterator = this) = new {
    val bit = _bit
    val wit = _wit
  } with WigIterator with FilteredBlockIterator[WigUnit, BedLine]

  def filterWithBed(_bed: BedIterator):WigIterator = filterWithBed(_bed,this)
...
object WigIterator {
  ...
  case class VariableStep(chr: String, span: Int, lines: Array[(Int, Double)]) extends WigUnit {
    ...
    override def toString: String =
      s"variableStep\tchrom=$chr" + {
        if (span != 1) s"span=$span" else ""
      } +
    ...
    def interSection[T <: Block](that: T) =
      that match {
        case BedLine(_, _start, _end, _, _, _, _, _, _, _, _, _) if this.hasIntersection(that) =>
          Some(VariableStep(chr, span, lines.dropWhile(_._1 < _start).takeWhile(_._1 < _end)).asInstanceOf[this.type])
        case _ => None
      }
  }
    object VariableStep {
    ...
    case class FixedStep(chr: String, start: Int, step: Int, span: Int, lines: Array[Double]) extends WigUnit {
      ...
      def interSection[T <: Block](bed: T) = throw new UnsupportedOperationException
